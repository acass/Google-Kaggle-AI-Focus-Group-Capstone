# Kaggle Writeup: Synthetic Market Intelligence Platform
...
The `build_graph` factory accepts a list of `AgentPersona` TypedDicts and generates node identifiers dynamically (e.g., `independent_skeptical_investor`, `vote_ux_researcher`). This means adding a new persona requires only adding a new entry to the persona registry — the graph topology wires itself.

### State Threading

All session data flows through a single `FocusGroupState` TypedDict that is threaded through the workflow via the ADK state mechanism. The fields that accumulate across nodes (stream events, citations, independent responses, discussion responses, and scores) use Python's `Annotated[list, operator.add]` pattern, which tells ADK to merge incoming lists additively rather than overwriting. This is what makes parallel fan-out safe: two participant nodes writing to `independent_responses` simultaneously will have their outputs merged cleanly without race conditions.

### Real-Time Streaming

Sessions run as FastAPI `BackgroundTask` instances. A `GET /sessions/{id}/stream` endpoint exposes an SSE generator that first replays all events already recorded (so a client that connects late or reconnects still sees the full history), then polls every 500ms for new events until the session completes. The Flutter client uses the browser-native `EventSource` — not a polling client, not a WebSocket — because `EventSource` handles reconnection and text encoding correctly without any custom implementation.

The `SessionNotifier` (Riverpod `Notifier`) on the frontend is a state machine that folds each incoming `StreamEvent` into the displayed session state. Agent messages, phase transitions, score updates, and the final report each advance distinct state fields. The UI renders from this state reactively.
...
Every agent that calls out a factual claim — market size figures, competitor information, technical standards — has access to two tools: Google Search (for live web retrieval) and `url_context` (for reading specific URLs). A third tool, `record_citation_tool`, is a lightweight ADK `FunctionTool` that appends `{title, url, excerpt}` records to the shared session state. The Synthesizer reads these citations and includes them in the final report. The Flutter client surfaces them as a scrollable citation list in the report view.

The grounding step was added late in development and had a meaningful qualitative impact: without it, agents made confident claims about market size and competitive landscape that were plausible but unverifiable. With it, the Synthesizer can say "according to [source], the addressable market is estimated at $X" rather than generating a number from training weights.

### Report Export

Once a session completes, the user can export the full report as PDF or DOCX. Both builders run entirely client-side in the browser — `pdf_builder.dart` using the `pdf` and `printing` packages, `docx_builder.dart` using hand-rolled OOXML zipped with the `archive` package. No server-side generation, no stored files. The decision to do this client-side was practical: it avoids storing generated documents on the server and eliminates a whole class of async job management complexity.

---

## Project Journey

The project started with a simpler premise: what if a single AI could play multiple characters in a discussion? The early prototype used a Next.js frontend and a sequential loop that prompted a single Gemini instance to respond as each persona in turn. The output was fluent but unconvincing — the responses converged quickly because each turn conditioned on the previous one, and the "characters" shared the same underlying reasoning distribution.

Replacing the sequential loop with a true parallel multi-agent graph — where each persona is a separate agent instance running concurrently and independently — changed the output qualitatively. The disagreements became sharper because agents genuinely could not see each other's Round 1 responses. Marcus Chen's critique of the business model no longer had to implicitly acknowledge that Zoe Park had already praised the novelty; he arrived at his position independently.

The frontend migration from Next.js to Flutter Web was motivated by the real-time streaming requirement. The Next.js implementation used polling, which produced a jerky, batched display that undermined the "watching a debate unfold" experience. Flutter's reactive model with Riverpod and native `EventSource` made the streaming feel genuinely live.

The security plugins were the last major addition and came from a specific concern: in a system where agents have web access, what stops a compromised or mis-prompted agent from making dozens of search calls and consuming disproportionate resources? The AGBOM-based monitoring and stateful quarantine were the answer. The `security_test_mode` flag in the session creation API lets you deliberately trigger the quarantine to test the defense in isolation.

The citation tracking system went through three iterations. The first version let agents mention sources inline in their text with no structured capture — citations were lost in prose and the Synthesizer could not aggregate them. The second version used a structured output schema for citations but required agents to emit all citations at the end of their response, which broke the natural flow of multi-tool calls. The final version uses an explicit `record_citation_tool` that agents call as a side effect when they find a relevant source — the same pattern as a human researcher bookmarking a tab.
...
The system works today. You can give it a pitch deck concept, and three minutes later have a structured report with a viability score, a consensus confidence interval, identified strengths and risks, specific action items, and cited sources. That is a business output, not a demo.

---

## What Is Next

The most obvious extension is session persistence and comparison — running the same topic through the same panel at different points in time to track how the consensus shifts as the market develops. A second natural direction is custom persona configuration: letting users define their own panel members with specific domains, biases, and scoring weights rather than selecting from the built-in five. A third is vertical specialization — a healthcare panel (regulatory expert, clinician, patient advocate, payer), a consumer hardware panel, an enterprise SaaS panel — where the personas encode domain knowledge that the generic panel lacks.

The underlying ADK workflow graph is already parameterized by persona list, so all three of these are additive changes that do not require restructuring the core architecture.
